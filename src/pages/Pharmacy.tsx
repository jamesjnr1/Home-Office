...
const features = [
  { title: 'Extended Hours', desc: `Open ${business.hours.toLowerCase()}.` },
  { title: 'Friendly Walk-In Service', desc: 'No appointment needed — walk in and our pharmacy team will help you right away.' },
  { title: 'Verified Medications', desc: 'Genuine, quality-assured medicines dispensed by our pharmacy team.' },
  { title: 'Refill Reminders', desc: 'Ask us to remind you when it is time to refill.' },
];

const pharmacyServices = [
  { title: 'Prescription Filling & Refills', desc: 'Bring in your prescription or an existing bottle and we will take it from there.' },
            ...
            <div>
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                {features.map((f) => (
                  <div
                    key={f.title}
                    className="border-l-2 border-ink-200 py-1 pl-4 transition-colors duration-300 hover:border-brand-600"
                  >
        ...
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          ...
          <div ref={servicesRef} className="mt-14 grid gap-6 sm:grid-cols-2">
            {pharmacyServices.map((s, i) => (
              <div
                key={s.title}
                className={`reveal ${servicesVisible ? 'is-visible' : ''} border border-ink-100 bg-white p-6 transition-colors duration-300 hover:border-brand-200`}
