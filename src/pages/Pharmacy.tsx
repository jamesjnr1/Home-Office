    ...
    <div className="page-enter">
          ...
          <div ref={ref} className={`reveal ${visible ? 'is-visible' : ''} grid items-center gap-12 lg:grid-cols-2 lg:gap-16`}>
            <div className="relative">
              ...
              <div className="absolute -bottom-6 left-6 right-6 rounded-2xl bg-white p-5 shadow-xl ring-1 ring-ink-100 sm:left-8 sm:right-auto">
                <div className="flex items-center gap-3">
                  <Pill className="h-5 w-5 shrink-0 text-brand-600" strokeWidth={1.75} />
                  <div>
            ...
            <div>
              ...
              <div className="mt-8 grid gap-6 sm:grid-cols-2">
                {features.map((f) => (
                  <div key={f.title} className="border-t border-ink-200 pt-4">
                    <f.icon className="h-5 w-5 text-brand-600" strokeWidth={1.5} />
                    <h4 className="mt-2 text-sm font-semibold text-ink-900">{f.title}</h4>
      ...
      <section className="py-24 sm:py-32">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          ...
          />
          <div ref={servicesRef} className="mt-14 grid border-l border-t border-ink-200 sm:grid-cols-2">
            {pharmacyServices.map((s, i) => (
              <div
                key={s.title}
                className={`reveal ${servicesVisible ? 'is-visible' : ''} border-b border-r border-ink-200 bg-white p-6`}
                style={{ transitionDelay: `${i * 70}ms` }}
              >
                <s.icon className="h-6 w-6 text-brand-600" strokeWidth={1.5} />
